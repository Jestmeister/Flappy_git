...
def main_gameplay():
    ...
    step=0
    inputHistory = np.empty((4, 0), np.double)
    outputHistory = []
    outputHistory2 = []
    ...
    while True:
        ...
        step+=1
        #time.sleep(0.02)
        FBoutput=[0,0,2.5,0]#bird pos,bird vel, pipe1 x, pipe1 y, pipe 2 x, pipe 2 y
        #print("\n")
       # print("Step:"+str(step))
        ...
        for pipe in up_pips:
            if(pipeCounter<1):
                ...
                if(up_pips[pipeCounter]['x']-p_x  >= -game_image['pipe'][0].get_width()/2):
                    ...
                    FBoutput[2+2*pipeOutputCounter]=up_pips[pipeCounter]['x']/200
                    #print("Pipe "+str(pipeCounter+1)+" y pos"+str((up_pips[pipeCounter]['y']+low_pips[pipeCounter]['y'])/2+game_image['pipe'][0].get_height()/2))
                    FBoutput[3+2*pipeOutputCounter]=((up_pips[pipeCounter]['y']+low_pips[pipeCounter]['y'])/2+game_image['pipe'][0].get_height()/2)/100
                    pipeOutputCounter+=1
                pipeCounter+=1
        ...
        if(step>60 and step%10==0):
            for i in range(max(step-101,0),step-1):
                #print(inputHistory[:, i])
                optimizer = torch.optim.SGD(model.parameters(), lr=0.002, momentum=0.01)
                Qvalues = model(torch.FloatTensor(inputHistory[:, i], device=device))
                Qtarget=torch.FloatTensor([Qvalues[0].item(),Qvalues[1].item()])
        ...
        if cr_tst:
            #print('Crash!!!!')
            UR=0.1
            for i in range(max(step-41,0),step-1):
                #time.sleep(0.1)
                optimizer = torch.optim.SGD(model.parameters(), lr=0.02, momentum=0.01)
                UR=UR*1.3
                optimizer.zero_grad()
                #print(torch.FloatTensor(inputHistory[:, i], device=device))
                Qvalues = model(torch.FloatTensor(inputHistory[:, i], device=device))
                Qtarget=torch.FloatTensor([Qvalues[0].item(),Qvalues[1].item()])
                #print('\n reset')
                #print(Qvalues)
