...
def main_gameplay():
    ...
    while True:
        ...
        step+=1
        #time.sleep(0.02)
        FBoutput=[0,0,500,0]#bird pos,bird vel, pipe1 x, pipe1 y, pipe 2 x, pipe 2 y
        #print("\n")
       # print("Step:"+str(step))
        ...
        if(NN==False):        
            ...
            #ADD NN
            #print("NN run")
            Qvalues = model(torch.FloatTensor(FBoutput)/100)
        ...
        if(step>60 and step%10==0):
            for i in range(max(step-101,0),step-1):
                ...
                Qvalues = model(torch.FloatTensor(inputHistory[:, i], device=device)/100)
