...
def main_gameplay():
    ...
    while True:
        ...
        if(NN==False):        
            ...
            #ADD NN
            print("NN run")
            Qvalues = model(torch.FloatTensor(torch.rand(6, device=device)))
            Qvalues2 = model(torch.FloatTensor(torch.rand(6, device=device)))
            ...
            outputHistory2.append(Qvalues2)
        ...
        if cr_tst:
            ...
            for i in range(max(step-21,0),step-1):
                print(inputHistory[:, i])
                print(outputHistory[i])
                print(outputHistory2[i])
                print(outputHistory2[i][0])
                print(outputHistory2[i][1])
                if(outputHistory2[i][0]>outputHistory2[i][1]):
                    print('aaaaaaaa')
                    outputHistory2[i][0]=0
                else:
                    outputHistory2[i][1]=0
